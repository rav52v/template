...
public class ClickFunctions extends BaseFunction {
  ...
  public void click(WebElement element) {
    ...
    new WebDriverWait(driver.getDriver(), 7, 20)
  ...
  public void waitAndClick(WebElement element) {
    log.debug("Wait for element {" + getElementInfo(element) + "} to be clickable, then click it");

    changeImplicitlyWaitTime(0);
    new WebDriverWait(driver.getDriver(), 10, 10).until(ExpectedConditions
  ...
  public void waitAndClick(By by) {
    ...
    changeBackImplicitlyWaitTime();
    new Actions(driver.getDriver()).moveToElement(driver.getDriver().findElement(by)).click().perform();
  }
  ...
  public void clickAndWaitForElement(WebElement element, By locator) {
    try {
      click(element);
      changeImplicitlyWaitTime(0);
      new WebDriverWait(driver.getDriver(), 5, 30).until(ExpectedConditions
  ...
  public void clickNTimes(WebElement element, int number) {
    for (int i = 0; i < number; i++) click(element);
  }
